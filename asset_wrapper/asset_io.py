...
BACKUP_ROOT_NAME = "Asset Wrapper Backups"
# Recognized so assets created by older versions still behave correctly.
LEGACY_BACKUP_ROOT_NAMES = ("AM_Collectionize_Backups",)
...
def resolve_asset_library_dir(context, create=True):
    ...
    if directory is None:
        raise ValueError(
            ...
            "or set a custom asset library folder."
        )
    ...
    if not os.path.isdir(directory):
        raise ValueError(f"Asset library is not a folder: {directory}")
...
def _save_user_preferences():
    try:
        ...
        print(f"Asset Wrapper: could not save user preferences: {exc}")
...
def autodetect_project_library():
    ...
    if directory and os.path.isdir(directory):
        try:
            ...
            print(f"Asset Wrapper: could not register project asset library: {exc}")
...
def create_export_collection(asset_name, source_objects, pivot_matrix):
    ...
    if export_collection.asset_data:
        export_collection.asset_data.description = "Generated by Asset Wrapper"
...
def render_preview_png(collection, size=None):
    ...
    try:
        ...
        print(f"Asset Wrapper: preview generation failed: {exc}")
        return None
...
if collection is not None:
    ...
    if pack_textures:
        try:
            ...
            print(f"Asset Wrapper: packing textures failed: {{exc}}")
...
"""
    ...
    try:
        handle = tempfile.NamedTemporaryFile(
            mode="w",
            suffix="_asset_wrapper_inject.py",
            encoding="utf-8",
            delete=False,
        )
        with handle:
        ...
        print(f"Asset Wrapper: preview injection skipped: {exc}", file=sys.stderr)
...
def _render_collection_preview(collection, size):
    ...
    scene = bpy.data.scenes.new("AW_Preview_Temp")
    camera_data = bpy.data.cameras.new("AW_Preview_Cam")
    camera = bpy.data.objects.new("AW_Preview_Cam", camera_data)
    fd, render_path = tempfile.mkstemp(suffix="_aw_preview.png")
    os.close(fd)
...
def is_user_facing_collection(collection):
    if collection.library is not None:
        return False
    if collection.name in _backup_root_names():
        return False

    for backup_root in _existing_backup_roots():
        if _collection_contains(backup_root, collection):
            return False

    return True


def _backup_root_names():
    return (BACKUP_ROOT_NAME,) + LEGACY_BACKUP_ROOT_NAMES


def _existing_backup_roots():
    roots = []
    for name in _backup_root_names():
...
def refresh_asset_browsers(context):
    try:
        ...
        print(f"Asset Wrapper: asset browser refresh skipped: {exc}")
...
def _ensure_backup_root(scene):
    ...
    if root is None:
        # Reuse a legacy backup root if one exists in this file.
        for name in LEGACY_BACKUP_ROOT_NAMES:
